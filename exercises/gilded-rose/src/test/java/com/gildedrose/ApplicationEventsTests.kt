...
class ApplicationEventsTests {
    ...
    fun `every request raises an event`() {
        ...
        val httpEvent = events.firstNotNullOfOrNull { it as? HttpEvent }
            ?: fail("event not raised")
        assertEquals(GET.name, httpEvent.method)
        assertEquals("/", httpEvent.uri)
        assertEquals(OK.code, httpEvent.status)
    }
}
